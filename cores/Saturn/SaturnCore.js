  ...
  pkg: {
    ...
    start: async function (d) {
      try {
        ...
        let Package = await import(d.PKG_URL);
        ...
        if (Package.default) {
          try {
            ...
            console.debug(
              `%cWait, ${d.name} (${PID}) is a library! I won't reserve as process.`,
            ...
            function Returns(obj) {
              console.debug(
                `%c${d.name}: ${PID} returned early!`,
                "font-weight:bold;color:red"
              );
              console.debug(
                `${d.name} (${PID}) returns pl of`,
                JSON.stringify(Core.processList, null, 2)
              );
            ...
            console.debug(
              `${d.name}: ${PID}`,
              "finished running.. the pl is",
              JSON.stringify(Core.processList, null, 2)
            );
            window.dispatchEvent(
              new CustomEvent("saturn-core.package.run", {
                detail: {
                  Package,
                  PROCESS_NAME,
                  PID,
    ...
    cacheRun: async (category, author, name, args, owned_by) => {
      let PKG_URL = `./packages/${category}/${author}-${name}.js`;
      // console.debug('Starting', PKG_URL);
      let cachedPackage = Core.pkgCache.find(
        (e) => e.item === `./packages/${category}/${author}-${name}.js`
      );
...
console.log(
  "%cHey!\n%cDon't paste anything in here.\nEntering any code into this console can leak your account details and/or files to external sources. Use it at your own risk.%c\n\nIf you %cdo %cknow what you're doing, you can contribute to this project at https://github.com/zeondev/saturn/",
  "text-shadow: 0 0 10px red;color:red;font-size: x-large",
